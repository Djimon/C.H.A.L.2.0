...
Skills are considered tools that enable build expression.
They define what a hero can do at any given moment, but they do not permanently define who the hero is.

Skill choices are expected to change frequently over time, while a hero’s long-term identity is shaped through progression systems.

\paragraph{Terminology (Skill System)}
\begin{tabular}{l p{11.5cm}}
	\hline
	\textbf{Term} & \textbf{Definition} \\
	\hline
	Skill &	A usable ability for heroes or enemies, defined by a \texttt{SkillData} instance.  
	A Skill describes \emph{what happens} when executed, but not how it is obtained or progressed. \\	

	Skill Module &	A craftable and equippable item that grants access to a Skill.  
	Modules define availability and configuration, not execution logic. \\
	
	SkillData &	The static, immutable definition of a Skill, including impacts, tags and execution rules.  
	SkillData does not change during runtime. \\
	
	Impact &	An atomic effect produced during Skill execution (e.g.\ damage, healing, applying a status).  
	A single Skill usually consists of multiple Impacts. \\
	
	Status Effect &	A time-based or conditional effect applied by an Impact.  
	Status Effects persist independently of the Skill that created them. \\
	
	ModifierData &	A data-driven rule that modifies values during Skill resolution.  
	Modifiers are global in nature and may affect multiple Skills simultaneously. \\
	
	Skill Execution &	The runtime process of resolving a Skill, including targeting, hit checks, impact resolution and modifier application. \\
	\hline
\end{tabular}
...
Skills do not primarily scale through level-based stat growth. Their long-term power, behaviour changes and specialization are mainly driven by Hero Orbit progression, with SkillTier and Gear acting as secondary multipliers.

\subsubsection{Skill Lifecycle Overview}


The lifecycle of a skill is intentionally modular and iterative. Skills are not permanent character traits, but crafted and replaceable tools that evolve alongside hero progression.

\begin{itemize}
	\item \textbf{Unlock} — Skill frames, tiers, or module categories are unlocked via Research or progression systems.
	\item \textbf{Craft} — A concrete skill module is crafted, defining its base effects, tags, and parameters.
	\item \textbf{Slot} — The crafted skill module is equipped into a hero loadout and becomes active.
	\item \textbf{Scale} — The skill scales through hero stats, orbit modifiers, gear, and external effects.
	\item \textbf{Replace} — Skills can be replaced, or re-crafted without affecting hero identity.
\end{itemize}
...
\subsection{Scope for the Skill System}
