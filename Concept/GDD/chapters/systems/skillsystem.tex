...
\begin{itemize}
	...
	\item archetypes define \textbf{how} it feels and can change impacts and effects (e.g. same Module might be a burning nova for an Arcanist and a physical shockwave for a Vanguard) $\rightarrow$ (see Section \ref{subsec:skill-families-profiles})
	\item Orbits, Gear and Runes further modify behaviour and scaling.
\end{itemize}

\paragraph{Module Profiles}
Not every Archetpye has a different representation of the Skill. there are 3 main profiles: \textbf{Melee}, \textbf{Ranged}, \textbf{Caster} (and Minion/Spawn as special Role)

\subparagraph*{Naming through damage type and Archetype-profile} 
General naming rule: prefix (dmg-type) + Module-Name + Suffix (profile). \\
...
\hfill
\begin{minipage}[t]{0.48\textwidth}
	\textbf{Profile Suffixes}\\
	
	\begin{tabular}{r| l}
		\textbf{Profile}    & \textbf{Suffix}     \\
		\hline
		Melee        & \glqq\textit{of Ferocity}\grqq   \\
		Ranged       & \glqq\textit{of Precision}\grqq  \\
		Caster       & \glqq\textit{of Sorcery}\grqq    \\
		Minion/Spawn & \glqq\textit{of Dominion}\grqq   \\
	\end{tabular}
\end{minipage}

\vspace{1em}

\noindent\textbf{Example Skill Module Names}

\begin{itemize}
	\item \glqq Blazing Gyro-Module of Ferocity\grqq
	\item \glqq Static Vortex-Module of Precision\grqq
	\item \glqq Venomous Graviton-Module of Dominion\grqq
	\item \glqq Glacial Pulse-Module of Sorcery\grqq
	\item \glqq Radiant Burst-Module of Precision\grqq
\end{itemize}

\textbf{General summary}: The module defines the basis, the archetype defines certain overrides, whereby not every archetype always has its own override; some archetypes share overrides when appropriate.
