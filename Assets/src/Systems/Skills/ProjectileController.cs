...
/// <param name="life">The lifespan of the projectile.</param>
        public void Init(SkillInstance inst, EffectReceiver src, EffectReceiver tgt, Vector3 dir, float projSpeed, float life)
        {
            ...
            DebugManager.Log($"[Projectile] Spawned {inst.skillModule.DisplayName} from {src} towards {tgt}", DebugManager.EDebugLevel.Test, "Projectile");
        }
        ...
        private void Check_LifetimeExpiration()
        {
            if (lifespan <= 0f)
            {
                DebugManager.Log($"[Projectile] {skill.skillModule.DisplayName} expired before hitting", DebugManager.EDebugLevel.Dev, "Projectile");
                Destroy(gameObject);
            }
        }
