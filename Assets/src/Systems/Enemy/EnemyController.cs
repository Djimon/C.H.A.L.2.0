        ...
        private void Advance_CastTimeOrFinish(float dt, HeroController heroCtrl)
        {
            ...
            if (_castRemaining <= 0f)
            {
                ...
                if (heroCtrl != null && heroCtrl.IsAlive)
                {
                    ...
                    if (dist <= (GameManager.Instance.BalanceConfig.GetRangeValue(_currentSkill.Range)))
                    {
                ...
                {
                    DebugManager.Log($"Targeting | Enemy has no valid target for {_currentSkill.skillModule.DisplayName}.", DebugManager.EDebugLevel.Dev, "Combat");
                }
