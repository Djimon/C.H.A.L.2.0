...
/// <param name="completedNodeIds">The collection of completed node IDs.</param>
        public void RebuildFrom(IEnumerable<CodexDeedDef> allNodes, IEnumerable<string> completedNodeIds)
        {
            Clear();
            ...
            // Lookup id â†’ def
            var byId = new Dictionary<string, CodexDeedDef>(StringComparer.Ordinal);
...
/// <param name="resetExisting">Indicates whether to clear existing entries.</param>
        public void InitializeCatalog(IEnumerable<CodexDeedDef> allNodes, bool resetExisting = false)
        {
