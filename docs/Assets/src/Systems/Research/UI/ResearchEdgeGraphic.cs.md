...
```markdown
# Purpose
- Defines a custom UI graphic component for rendering an orthogonal line with a small Bezier corner as a mesh.
...
- Types
  - `public sealed class ResearchEdgeGraphic : MaskableGraphic`
    - Public fields/properties:
      ...
      - `public float cornerRadius` - Radius of the corner (range 0 to 1).
...
- The `OnPopulateMesh` method generates the mesh for the line based on the `start`, `end`, `thickness`, `cornerRadius`, and color properties.
...
edgeGraphic.end = new Vector2(100, 100);
...
# Unknowns
- No information on how this component interacts with other systems or components outside of its own implementation.
```
