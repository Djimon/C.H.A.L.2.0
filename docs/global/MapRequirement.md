...
# Public API
- Namespace: Not explicitly defined; likely part of `CHAL.Data`.
- Types:
...
- `killsByTag` can be null; checks are performed to avoid null reference exceptions.
- The method `ValidateSoft` uses an action delegate for warning messages, allowing for flexible handling of warnings.
...
- The context in which `ResearchRequirement` is used is not specified.
