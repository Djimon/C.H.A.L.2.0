...
- Types
  - `public class ExampleSaveCustom : MonoBehaviour`
    ...
    - Public methods:
      ...
      - `void SetScore(string score)`: Sets the score from input string.
      - `void SetHighScore(string highScore)`: Sets the high score from input string.
...
- Uses `SaveGame.Save` and `SaveGame.Load` methods for data persistence.
...
# Unknowns
- The implementation details of `SaveGame.Save` and `SaveGame.Load` methods.
- The behavior of `SerializerDropdown.Singleton.ActiveSerializer`.
