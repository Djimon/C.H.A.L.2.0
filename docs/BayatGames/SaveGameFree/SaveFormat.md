...
- The `Save()` method saves the object's position, rotation, and scale based on the specified identifiers and flags.
- The `Load()` method loads the object's position, rotation, and scale, applying defaults if no saved data exists.

# Constraints & Failure Modes
- Requires valid identifiers for saving data.
- If `resetBlanks` is true, resets empty fields to default values on `Awake()`.
...
# Unknowns
- Specific implementations of `ISaveGameSerializer` and `ISaveGameEncoder` are not detailed in this file.
- The behavior of `SaveGame.Save<T>()` and `SaveGame.Load<T>()` methods is not defined in this file.
