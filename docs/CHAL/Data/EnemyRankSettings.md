...
# Purpose
- Defines a configuration asset for game balance settings, including loot parameters, enemy settings, skill ranges, and economy settings.
...
- The `GetMultiplier`, `GetScaling`, and `GetRangeValue` methods default to specific values if an unrecognized rank or range is provided.
