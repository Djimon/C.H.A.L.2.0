...
- The `GetRangeValue(SkillRange range)` method retrieves the range value based on the specified skill range.
- The `GetMultiplier(EnemyRank rank)` method retrieves the loot rank multiplier based on the specified enemy rank.
- The `GetScaling(EnemyRank rank)` method retrieves the rank scaling based on the specified enemy rank.
