...
# Purpose
- Defines the `AttackDef` class as a ScriptableObject for configuring attack definitions in the game.
- Contains properties for attack identity, damage types, cooldown, tags, animation type, and visual effects.
...
- The `DamageEntry` struct allows for the creation of damage types with multipliers.

# Constraints & Failure Modes
- No explicit guards or null handling noted.
- The `AttackDef` class relies on Unity's serialization for its fields.
...
- The implications of the deprecation warning for `AttackDef` are not detailed.
