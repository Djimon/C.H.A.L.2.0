...
- Provides structured settings for game balance, including loot, enemy scaling, skill ranges, and economy.
- The `GetMultiplier` method in `LootRankMultipliers` and `GetScaling` method in `EnemyRankSettings` return values based on the provided `EnemyRank`.
