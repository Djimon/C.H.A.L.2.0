...
- Types
  - `public class InvDnDProvider : MonoBehaviour`
    ...
    - Public methods:
      - `private void OnValidate()` - Rebuilds the service in the editor if the domain is set.
...
- The `OnValidate` method is only called in the Unity editor, not during runtime.
