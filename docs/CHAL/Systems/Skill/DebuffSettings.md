...
- `BaseDuration` and `BaseMaxStacks` must be non-negative; defaults to 0 and 1 respectively if not set.
- `CurrentStacks` is initialized to 1 and cannot exceed `_currentMaxStacks`.
...
# Unknowns
- The implementation details of `ActiveStatusEffect`, `EffectReceiver`, and `ModifierData` are not provided in this file.
- The behavior of `StackingMode` is not defined in this file.
