...
- Types
  - `sealed class LootDrop`
    - Public fields/properties:
      ...
      - `float[] chancesArray` - Array of chances for multiple drop options (null if using `chance`).
...
- `chance` and `chancesArray` cannot be used simultaneously; one must be null.
