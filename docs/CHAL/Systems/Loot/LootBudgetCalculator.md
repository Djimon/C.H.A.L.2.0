...
- Types
  - **public static class** `LootBudgetCalculator`
    ...
    - **private static float** `getMultiplierFromMapDifficulty(MapDifficulty difficulty)`
      - Returns a multiplier based on the specified map difficulty.
...
# Unknowns
- The structure and initialization of `BalanceManager.Instance.Config` and its properties are not defined in this file.
