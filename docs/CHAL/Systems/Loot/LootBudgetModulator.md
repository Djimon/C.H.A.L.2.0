...
# Purpose
- Defines a static class `LootBudgetModulator` for calculating drop chance modifiers based on budget overflow.

# Public API
- Namespace: `CHAL.Systems.Loot`
- Types
  - **static class** `LootBudgetModulator`
    - **Public methods**
...
- Calculates overflow when the budget is exceeded and applies an exponential decay based on a configured beta value.
- Determines a floor value based on the rarity type, ensuring the returned modifier does not fall below this floor.
...
# Unknowns
- The structure and initialization of `BalanceManager.Instance.Config.loot.budget` and `BalanceManager.Instance.Config.loot.floors` cannot be determined from this file.
