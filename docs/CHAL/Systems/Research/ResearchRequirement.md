...
- Types
  - **public class ResearchRequirement**
    ...
    - Public methods:
      ...
      - `bool IsEmpty()`: Checks if the current instance is empty, meaning it has no active waves, maps, or kills.
...
- `ValidateSoft` method checks for negative values in the fields and triggers warnings if any invalid conditions are found.
...
- The `killsByTag` list can contain null entries, which are handled in the `ValidateSoft` method.
