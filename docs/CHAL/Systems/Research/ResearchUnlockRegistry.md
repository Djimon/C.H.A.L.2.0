...
- `RebuildFrom(...)`: Clears existing data, initializes the catalog, and applies unlocks based on completed nodes. Logs warnings for null arguments and issues during processing.
- `InitializeCatalog(...)`: Populates the catalog with target IDs from provided nodes, logging the number of added IDs.
- `ApplyNodeUnlocks(...)`: Applies unlocks from a node, updating internal collections and the catalog. Logs warnings for empty target IDs and unknown unlock types.
...
## Unknowns
- The structure and properties of `ResearchNodeDef` and `ResearchUnlock` are not defined in this file.
