...
# Purpose
- Defines the `ItemRegistry` class, which manages item definitions and their validation within the game.
...
- Validation reports are generated and saved to a CSV file if inconsistencies are found during loading.

# Constraints & Failure Modes
- The `Reload` method handles null or empty item IDs and duplicates, logging warnings for each issue.
- The `LoadModulePartMap` method warns if the ModulePartMap is not found.
